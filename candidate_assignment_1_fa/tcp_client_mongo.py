...
class TCPclient():
    ...
    def voting(self, user_info):
        ...
        if user_info['point'] != 0:
            ...
            user_info_update = self.read_l_data_after_vote(client)
            print("client user info update : ---> ", user_info_update)
            self.user_option(user_info, client)
        else:
            print("Not enough point to vote: ")
            self.user_option(user_info, client)
    ...
    def read_l_data_after_vote(self, client):
        user_info_update = {}
        client = self.client_runner()
        info = client.recv(4096)

        current_login_data = json.loads(info.decode("utf-8"))
        user_info_update = current_login_data

        client.close()
        print("client : ",user_info_update)
